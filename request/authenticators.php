...
if (!empty($errors)){
    ...
    $SQLUpdate = $odb -> prepare("UPDATE `users` SET `google` = :google WHERE `username` = :username AND `ID` = :id");
    $SQLUpdate -> execute(array(':google' => '',':username' => $_SESSION['username'], ':id' => $_SESSION['ID']));
    $SQLUpdate = $odb -> prepare("UPDATE `users` SET `ah` = :ah WHERE `username` = :username AND `ID` = :id");
    $SQLUpdate -> execute(array(':ah' => 0,':username' => $_SESSION['username'], ':id' => $_SESSION['ID']));
    session_start();
