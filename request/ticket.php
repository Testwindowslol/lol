...
if ($csrf != $_SESSION['token'])
    ...
    $SQLGetTickets = $odb -> query("SELECT * FROM `tickets` WHERE `id` = $idzzzdijdsduhusydhudhsudihuisdheiufhefgydiuhejfeuihfyuegfheudheufeghdeidnehfgbebuidhned");
...
if ($username != $_SESSION['username'])
    ...
    $i = 0;
    $SQLGetMessages = $odb -> query("SELECT * FROM `messages` WHERE `ticketid` = '$idzzzdijdsduhusydhudhsudihuisdheiufhefgydiuhejfeuihfyuegfheudheufeghdeidnehfgbebuidhned' ORDER BY `messageid` DESC LIMIT 1");
